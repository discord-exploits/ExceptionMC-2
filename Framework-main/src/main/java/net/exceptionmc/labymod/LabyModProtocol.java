...
public class LabyModProtocol {
    ...
    private static void writeString(ByteBuf buf, String string) {
        ...
        if (aByte.length > Short.MAX_VALUE) {
            throw new EncoderException("String too big (" + string.length() + " bytes, max " + Short.MAX_VALUE + ")");
        } else {
    ...
    public static String readString(ByteBuf buf, int maxLength) {
        ...
        if (i > maxLength * 4) {
            throw new DecoderException("Received buffer longer than allowed (" + i + " > " + maxLength * 4 + ")");
        } else if (i < 0) {
            throw new DecoderException("Received buffer is less than zero! Weird string!");
        } else {
            ...
            if (s.length() > maxLength) {
                throw new DecoderException("Received string longer than allowed (" + i + " > " + maxLength + ")");
            } else {
                return s;
            }
        }
